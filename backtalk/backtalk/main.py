...
def _deny_pending(reason=_INTERRUPT_ANSWER):
    ...
    if f is not None and not f.done():
        f.set_result(reason)
def _human_what(tool, tool_input, ctx=None):
    """The SHORT spoken form, built for a person who has never seen a
    terminal: plain words, no paths, no syntax."""
    d = tool_input or {}
    if tool in ("Write", "Edit", "MultiEdit", "NotebookEdit", "write_file", "edit_file"):
        path = str(d.get("path") or d.get("file_path") or d.get("notebook_path")
                   or "a file").replace("\\", "/")
        name = path.rsplit("/", 1)[-1]
        verb = "edit" if ("Edit" in tool or "edit" in tool) else "create or change"
        if name.endswith(".md"):
            return f"{verb} a note called {name[:-3]}"
    ...
    if tool == "append_memory":
        note = d.get("note_name", "memory")
        return f"save a new entry to {note}"
...
def _full_detail(tool, tool_input, ctx=None):
    d = tool_input or {}
    if tool in ("Bash", "run_command"):
        cmd = " ".join(str(d.get("command", "")).split())
        chained = any(m in cmd for m in _CHAIN_MARKS)
    ...
    if tool in ("Write", "Edit", "MultiEdit", "NotebookEdit", "write_file", "edit_file"):
        ...
        return f"{'edit' if ('Edit' in tool or 'edit' in tool) else 'write'} the file {name}"
    if tool == "append_memory":
        return f"append '{d.get('entry', '')[:50]}' to {d.get('note_name', 'memory')}"
...
def make_permission_gate(mouth):
    async def gate(tool, tool_input, ctx=None):
        if _AUTOAPPROVE["on"]:
        ...
        if not _PERM["hinted"]:
            _PERM["hinted"] = True
            ask += (" And any time you're done with these checks, say stop asking for permission.")
        mouth.say(ask)
        answer = None
        try:
                    ...
                    try:
                        ...
                        if loop.time() >= deadline:
                            ...
                            return PermissionResultDeny(
                                behavior="deny",
                                message="No spoken answer within the timeout; the action was not approved.",
                                interrupt=False)
                        if not mouth.speaking:
                            signals.set_state("listening")
                if (got != _INTERRUPT_ANSWER and _norm_speech(got) in _DETAILS):
                    log("[perm]   details requested")
                    mouth.say(f"The details: I want to {detail}. Yes or no?")
                    deadline = loop.time() + PERM_TIMEOUT_S
                    continue
                answer = got
        finally:
        ...
        if answer == _INTERRUPT_ANSWER:
            ...
            return PermissionResultDeny(
                behavior="deny",
                message="Interrupted by the user; the turn is being cancelled.",
                interrupt=False)
